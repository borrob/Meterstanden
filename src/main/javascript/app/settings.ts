export class Settings {

	public static readonly maandverbruikURL = 'http://localhost:8080/meterstanden/maandverbruik';
	public static readonly maandverbruikUtilURL = 'http://localhost:8080/meterstanden/util';
	public static readonly overzichtmaandverbruikURL = 'http://localhost:8080/meterstanden/overzichtmaandverbruik';
	public static readonly jaarverbruikURL = 'http://localhost:8080/meterstanden/util?q=jaarverbruik';
	public static readonly metersoortenURL = 'http://localhost:8080/meterstanden/metersoorten';
	public static readonly meterstandenURL = 'http://localhost:8080/meterstanden/meterstanden';

	public static readonly color1='rgb(10,10,10)';
	public static readonly color1transparent='rgba(10,10,10,0.2)';
	public static readonly color2='rgb(0,0,200)';
	public static readonly color2transparent='rgba(0,0,200,0.2)';
	public static readonly color3='rgb(0,200,0)';
	public static readonly color3transparent='rgba(0,200,0,0.2)';

	public static readonly graphXlabel=["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

}
